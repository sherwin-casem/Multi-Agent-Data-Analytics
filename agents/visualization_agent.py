...
class VisualizationAgent:
    ...
    def generate_chart(self, data: Union[pd.DataFrame, str, None], query: str, context: Any = None, chat_history: List[Dict[str, str]] = []) -> Tuple[Any, str]:
        """
        ...
        if isinstance(data, pd.DataFrame):
            ...
            if chart_config.get("type") == "none": # Check if analyze_query decided against a chart
                ...
                explanation = self.gemini_agent.generate_response(data if isinstance(data, pd.DataFrame) else None, query, context=context, chat_history=chat_history)
    ...
    def explain_visualization(self, data: Union[pd.DataFrame, str], query: str = "Explain the chart", context: Any = None, chat_history: List[Dict[str, str]] = []) -> str:
        """
        ...
        if isinstance(data, pd.DataFrame):
            ...
            logging.error("Invalid data type passed to generate_response. Expected DataFrame or None.")
            return "Error: Unsupported data type for explanation."
